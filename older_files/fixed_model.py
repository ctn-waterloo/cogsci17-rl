...
def correct_mapping(x):
    ...
    closest_state = find_closest_vector(x, index_to_state_vector)

    if closest_state == 0:
        return np.concatenate((index_to_state_vector[1]*.7 + index_to_state_vector[2]*.3, index_to_state_vector[1]*.3 + index_to_state_vector[2]*.7))
    else:
        # Always return to state 0 at this point
        return np.concatenate((index_to_state_vector[0], index_to_state_vector[0]))

# Initial 50-50 function
def initial_mapping(x):

    closest_state = find_closest_vector(x, index_to_state_vector)

    if closest_state == 0:
        return np.concatenate((index_to_state_vector[1]*.5 + index_to_state_vector[2]*.5, index_to_state_vector[1]*.5 + index_to_state_vector[2]*.5))
    else:
        # Always return to state 0 at this point
        return np.concatenate((index_to_state_vector[0], index_to_state_vector[0]))
...
with model:
    ...
    if learning:
        ...
        conn = nengo.Connection(model.state_ensemble, model.combined_probability,
                                function=function,
                                learning_rule_type=nengo.PES(pre_synapse=z**(-int(time_interval*1000)), learning_rate=learning_rate),
                               )
    else:
        with cfg:
            model.state_ensemble = nengo.Ensemble(n_neurons=DIM*50,dimensions=DIM)
            nengo.Connection(model.state_ensemble, model.combined_probability,
                             function=function,
                            )
    ...
    with cfg:
        ...
        model.value = nengo.Ensemble(200, 2)
